...
class MyExampleBuildCommand(sublime_plugin.WindowCommand): # TODO1  called from sbot_dev.sublime-project - fix/debug
    ...
    def run(self, lint=False, integration=False, kill=False):
        ...
        with self.panel_lock:
            ...
            # Enable result navigation. The result_file_regex does the primary matching, but result_line_regex is used
            # when build output includes some entries that only contain line/column info beneath a previous line
            # listing the file info. The result_base_dir sets the path to resolve relative file names against.
